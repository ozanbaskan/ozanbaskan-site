let rightNavPush = document.getElementById("pop-right-nav");
let rightNav = document.getElementById("right-nav");

rightNavPush.addEventListener("keyup", (event) => {
  // enter
  if (event.keyCode === 13) {
    // Cancel the default action, if needed
    event.preventDefault();
    // Trigger the button element with a click
    rightNavPush.click();
  }
});
rightNavPush.addEventListener("click", () => {
  if (rightNavPush.style.marginRight === "") {
    let push = 283;
    rightNavPush.style.marginRight = "283px";
    rightNav.style.marginRight = "0px";
    const animate = () => {
      if (push < 5) clearInterval(interval);
      rightNavPush.style.marginRight = push + "px";
      rightNav.style.marginRight = push - 283 + "px";
      push = push - 10;
    };
    let interval = setInterval(animate, 10);
    return;
  }

  if (rightNavPush.style.marginRight === "283px") {
    let push = 283;
    rightNavPush.style.marginRight = "283px";
    rightNav.style.marginRight = "0px";
    const animate = () => {
      if (push < 5) clearInterval(interval);
      rightNavPush.style.marginRight = push + "px";
      rightNav.style.marginRight = push - 283 + "px";
      push = push - 10;
    };
    let interval = setInterval(animate, 10);
    return;
  }

  if (rightNavPush.style.marginRight === "3px") {
    let push = 3;
    const animate = () => {
      if (push > 280) clearInterval(interval);
      rightNavPush.style.marginRight = push + "px";
      rightNav.style.marginRight = push - 283 + "px";
      push = push + 10;
    };
    let interval = setInterval(animate, 10);
    return;
  }
});
